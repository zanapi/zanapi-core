...
function PluginLoader(_app) {
    ...
    // Try to load global zanapi-plugins (if zanapi is installed locally it tries only local)
    this.detectPugins(__dirname);

    // Try to load local zanapi-plugins
...
/**
 * Detect modules start with "zanapi-plugin-".
...
PluginLoader.prototype.detectPugins = function(dir) {
    ...
    // Every dir start with "zanapi-plugin-", because for the tests of zanapi-plugin-test.
    var plugins = glob.sync(dir + '/zanapi-plugin-*');
    if (plugins.length === 0) {
        dir = path.join(dir, '..');
        if (dir === '/' || dir.substr(1) === ':\\')
            return;
